...
begin
  ...
  begin
    {frmCadastroVendas.queryVendasCOD_CLIENTE.Value := dbVendas.Fields[6]
      .AsInteger;}
      // Varre o Array de produtos
    for i := 0 to jsonProdutos.Size - 1 do
    begin
      if queryProdutos.Locate('COD_SITE', jsonProdutos.Get(i)
        .GetValue<integer>('product_id').ToString) then
      begin
        var
          insert: string;
        queryItensVendas.Active := true;
        queryItensVendas.Append;
        queryItensVendas.SQL.Clear;
        insert := 'INSERT INTO VENDAS_CLIENTES_ITENS(COD_VENDAS_CLIENTES_ITENS,DOCUMENTO,COD_PRODUTO,PROD_CLASSFISCAL, PROD_CLASSTRIB, VALORPRODUTO,QTDE) VALUES (27,1,'
          + jsonProdutos.Get(i).GetValue<integer>('sku').ToString + ',1,1,' +
          jsonProdutos.Get(i).GetValue<Double>('total').ToString + ',' +
          jsonProdutos.Get(i).GetValue<integer>('quantity').ToString+')';
        queryItensVendas.SQL.Add(insert);
        queryItensVendas.Open;
      end
      else
      begin
        ShowMessage('Produto ' + jsonProdutos.Get(i)
          .GetValue<integer>('product_id').ToString +
